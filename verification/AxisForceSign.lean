...
theorem radial_force_div_acceleration
 ...
 theorem axial_force_ratio_negative
    ...
    (hU : 0 < U) (hd : 0 < d) :
    -nu * NaturalAxisData.Z h j P eta * d / (L * A * U) < 0 := by
  ...
  · exact mul_neg_of_neg_of_pos (mul_neg_of_neg_of_pos (neg_neg_of_pos hnu) hzpos) hd
  · positivity
...
theorem actual_axis_force_ratio_negative
    ...
    -nu * NaturalAxisData.Z h j P eta * NaturalAxisData.d eta /
      (NaturalAxisData.L h eta * NaturalAxisData.A h *
       NaturalAxisData.U j eta) < 0 := by
...
theorem candidate_axis_coordinateEta_hasDerivAt
    (h q eta : ℝ) (hh : 0 < h) (hh1 : h < 1/2)
    (hq : 0 < q) (heta : eta ∈ Set.Ioo (-1 : ℝ) 1) :
    HasDerivAt (fun z : ℝ => SimilarityCoordinates.coordinateEta (2*h) (q*(1-eta^2),z))
      ((1-eta^2)*q^(-CoordinateAlgebra.D h)/NaturalAxisData.L h eta)
      (eta*q^(CoordinateAlgebra.D h)) := by
  have he := candidate_axis_inverse_coordinate h q eta hh hh1 hq heta
  have hqc := candidate_axis_coordinateQ_hasDerivAt h q eta hh hh1 hq heta
  have hp := (hasDerivAt_id (eta*q^CoordinateAlgebra.D h)).mul
    (hqc.rpow_const (p := -CoordinateAlgebra.D h) (Or.inl (by rw [he]; exact hq.ne')))
  rw [he] at hp
  have hfun : (fun z : ℝ => z * (SimilarityCoordinates.coordinateQ (2*h) (q*(1-eta^2),z))^(-CoordinateAlgebra.D h)) =
      (fun z : ℝ => SimilarityCoordinates.coordinateEta (2*h) (q*(1-eta^2),z)) := by
    funext z
    have hD : (1-2*h)/2 = CoordinateAlgebra.D h := by unfold CoordinateAlgebra.D; ring
    have hd : 0 < 1-eta^2 := by nlinarith [heta.1,heta.2]
    have hpos := (SimilarityCoordinates.coordinateQ_spec (by positivity : 0<2*h)
      (by linarith : 2*h<1) (p := (q*(1-eta^2),z)) (mul_pos hq hd)).1
    simp only [SimilarityCoordinates.coordinateEta,hD,Real.rpow_neg hpos.le,div_eq_mul_inv]
  have hpow : q^CoordinateAlgebra.D h*q^(-CoordinateAlgebra.D h-1)*q^CoordinateAlgebra.A h =
  ...
  have hL : NaturalAxisData.L h eta ≠ 0 := by
    unfold NaturalAxisData.L
    nlinarith
  simp only [Pi.mul_apply,id_eq] at hp
  change HasDerivAt (fun z : ℝ => z * (SimilarityCoordinates.coordinateQ (2*h) (q*(1-eta^2),z))^(-CoordinateAlgebra.D h)) _ _ at hp
  rw [hfun] at hp
  apply hp.congr_deriv
  calc
    _ = q^(-CoordinateAlgebra.D h) -
      (2*CoordinateAlgebra.D h*eta^2/NaturalAxisData.L h eta)*
        (q^CoordinateAlgebra.D h*q^(-CoordinateAlgebra.D h-1)*q^CoordinateAlgebra.A h) := by ring
    _ = _ := by
      rw [hpow]
      field_simp
      unfold NaturalAxisData.L CoordinateAlgebra.D
      ring

theorem axis_velocity_expression_axial_derivative
    (h j q eta : ℝ) (hh : 0 < h) (hh1 : h < 1/2)
    (hq : 0 < q) (heta : eta ∈ Set.Ioo (-1 : ℝ) 1) :
    HasDerivAt (fun z : ℝ =>
      (SimilarityCoordinates.coordinateQ (2*h) (q*(1-eta^2),z))^(-CoordinateAlgebra.A h) *
        (4*SimilarityCoordinates.coordinateEta (2*h) (q*(1-eta^2),z)+j))
      (q^(-1:ℝ)*(4*(1-eta^2)-2*CoordinateAlgebra.A h*eta*(4*eta+j))/NaturalAxisData.L h eta)
      (eta*q^CoordinateAlgebra.D h) := by
  have he := candidate_axis_inverse_coordinate h q eta hh hh1 hq heta
  have hD : (1-2*h)/2=CoordinateAlgebra.D h := by unfold CoordinateAlgebra.D; ring
  ...
  have hp := (hqd.rpow_const (p := -CoordinateAlgebra.A h) (Or.inl (by rw [he]; exact hq.ne'))).mul
    ((hed.const_mul 4).add_const j)
  rw [he,heta0] at hp
  have hpow1 : q^(-CoordinateAlgebra.A h-1)*q^CoordinateAlgebra.A h=q^(-1:ℝ) := by
    rw [← Real.rpow_add hq]
    congr 1
    ring
  have hpow2 : q^(-CoordinateAlgebra.A h)*q^(-CoordinateAlgebra.D h)=q^(-1:ℝ) := by
    rw [← Real.rpow_add hq]
    congr 1
  ...
  apply hp.congr_deriv
  calc
    _ = (-2*CoordinateAlgebra.A h*eta*(4*eta+j)/NaturalAxisData.L h eta)*
      (q^(-CoordinateAlgebra.A h-1)*q^CoordinateAlgebra.A h) +
...
theorem selected_base_material_trajectory
    ...
    HasDerivAt curve
      (SlowBorelBase.baseVelocity (FinalSlowBase.scales H v upper B) F.data.h
        W.axis.normalization (FinalSlowBase.coefficients H v) (t,curve t)) t := by
  intro curve
  have hd : 0 < 1-eta^2 := by nlinarith [heta.1,heta.2]
  have hq : 0 < (1-t)/(1-eta^2) := div_pos (by linarith) hd
  have he := selected_base_velocity_on_candidate_axis H v upper ((1-t)/(1-eta^2)) eta B hq heta
  have timeeq : 1-((1-t)/(1-eta^2))*(1-eta^2) = t := by
    field_simp
    ring
  rw [timeeq] at he
  change HasDerivAt _ _ t
  rw [he]
  have hz := root_axis_curve_hasDerivAt F.data.h W.axis.j eta t ht heta root
...
theorem actual_candidate_material_acceleration
    (B N0 : ℕ) (hN : ActualCarrierGeometry.geometricThreshold ≤ N0)
    (a : ℕ → ℝ) (ha : Filter.Tendsto a Filter.atTop Filter.atTop)
    (eta : ℝ) (heta : eta ∈ Set.Ioo (-1 : ℝ) 1)
    (root : NaturalAxisData.H CorrectionInitialization.ActualPrimary.h
    ...
    ∀ᶠ q : ℝ in 𝓝[>] (0 : ℝ),
    let t := 1-q*(1-eta^2)
    ProblemStatement.temporalDerivative u t (curve t) + ProblemStatement.advection u t (curve t) =
      AxisymmetricResidual.pack 0 0
        (CoordinateAlgebra.A CorrectionInitialization.ActualPrimary.h *
          NaturalAxisData.U CorrectionInitialization.ActualPrimary.nominal.axis.j eta /
          (1-eta^2) * q^(-CoordinateAlgebra.A CorrectionInitialization.ActualPrimary.h-1)) := by
  intro u curve
  filter_upwards [actual_candidate_terminal_base_germ B N0 hN a ha eta heta,
    actual_candidate_material_trajectory B N0 hN a ha eta heta root,
    actual_candidate_velocity_time_derivative B N0 hN a ha eta heta,
    self_mem_nhdsWithin] with q he hc hv hq
  dsimp only
  have hd : 0 < 1-eta^2 := by nlinarith [heta.1,heta.2]
  have ht : 1-q*(1-eta^2)<1 := by nlinarith [mul_pos hq hd]
  have hqt : (1-(1-q*(1-eta^2)))/(1-eta^2)=q := by field_simp; ring
  ...
  have hb := ((FinalSlowBase.velocity_smooth CorrectionInitialization.ActualPrimary.certificate
    CorrectionInitialization.ActualPrimary.modulation CorrectionInitialization.ActualPrimary.upper B).contDiffAt
      (hopen.mem_nhds (show (1-q*(1-eta^2),curve (1-q*(1-eta^2))) ∈ BaseResidual.past from
        ⟨ht,Set.mem_univ _⟩))).differentiableAt (by simp)
  have hu : DifferentiableAt ℝ u (1-q*(1-eta^2),curve (1-q*(1-eta^2))) :=
